...
// X to lose, Y to tie, and Z to win

const roundsButLikePartOne = rounds.map((round) => {
  let myMove = "";
  if (round === "A Z" || round === "B Y" || round === "C X") myMove = "Y";
  else if (round === "A X" || round === "B Z" || round === "C Y") myMove = "Z";
  else if (round === "A Y" || round === "B X" || round === "C Z") myMove = "X";
  return round[0] + " " + myMove;
});

// (1 for Rock, 2 for Paper, and 3 for Scissors)
// (0 if you lost, 3 if the round was a draw, and 6 if you won)

const scores = roundsButLikePartOne.map((round) => {
